...
$ ./run-tests.sh                         # PASS=30 FAIL=0 と出れば成功
```
...
$ ./run-tests.sh                     # PASS=30 FAIL=0 と報告。失敗があれば非ゼロ終了
$ TIMEOUT=600 ./run-tests.sh /tmp/result.txt      # 制限時間を延ばし、ログ先を指定
```

27 本のプログラムはそれぞれ翻訳され、コンパイルされ、実行され、そして
...
| sklearn `lasso_path`、暖かい開始 | 0.89 秒 |

冷たい者同士 — すべての lambda をゼロから解く、分割が異なる交差検証格子の
形 — では、翻訳カーネルは 1 コアで sklearn の約 22 倍、GPU バッチはさらに
その約 10 倍です。暖かく始める `lasso_path` は、各解を次の出発点として
再利用することで逐次の勝負に勝ちます。その梃子はバッチ並列と直交しており、
降下が正確に再開可能である以上、こちらのカーネルでも使えます。
sklearn の冷たい行は 256 回の当てはめからの推定です。nvcc とデバイスがある
環境で `run-tests.sh` が走らせる検査は、同じプログラムの小さな事例です。
...
回帰スイートの 30 本のうち 29 本が検査を通ります。通らない 1 本は遅延
