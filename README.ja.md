...
fold の Gram を alpha ごとに複製するため、複製が 512 MB を超えない間
だけ選ばれ、大きい p では CPU の warm パスが引き継ぎます。cv=5、
alpha 100 個、CPU 1 コアでの実測:

| n | p | `CovLassoCV` | sklearn `LassoCV` |
|---|---|---|---|
| 1,800 | 200 | 0.23 秒 | 0.20 秒 |
| 5,000 | 1000 | 1.1 秒 | 3.4 秒 |
| 100,000 | 200 | 0.44 秒 | 5.7 秒 |
| 100,000 | 500 | 0.90 秒 | 8.0 秒 |

n=100,000 では選ばれる alpha は同一、係数は 7e-7 で一致。小さい n では
準同点上で 1 格子点ずれることがあり、sklearn の許容誤差を締めれば当方側に
一致します。GPU と CPU は双方が走る領域で 1e-14 一致です。
