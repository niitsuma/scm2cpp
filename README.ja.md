...
あります: fold の訓練 Gram は**引き算**で得る(Gram は行について加法的
なので、G − Xf'Xf は fold 1 個分の費用。作り直しなら 4 fold 分)。
そして GPU では全 fold × 全 alpha が**単一起動の 1 スレッドずつ**になり、
各スレッドが自分の fold の Gram と自分の罰則を持ちます。cv=5、
alpha 100 個では皆速い(当方 CPU 0.12 秒、sklearn 0.11 秒)ですが、
探索を cv=10 × alpha 1000 個 — 独立な問題 1 万個 — に広げると単一
起動が効きます:

| ソルバ | cv=10、alpha 1000 個 |
|---|---|
| `CovLassoCV`、GPU 単一起動 | 1.2 秒 |
| sklearn `LassoCV` | 1.7 秒 |
| `CovLassoCV`、CPU 1 コア | 2.6 秒 |

GPU と CPU は 1e-14 で一致。sklearn とは準同点上で 1 格子点ずれること
があり、sklearn の許容誤差を締めれば当方側に一致します。
