...
;;[ja] ============================================================
;;[ja] アルゴリズム W の入口。流れ:
;;[ja]   1. トップレベル define を先に全部環境登録(相互再帰のため)
;;[ja]   2. infer を 3 周 — 単一化は単調なので、後の呼び出し側の情報が
;;[ja]      前の関数へ伝播する(square の x が後から double になる仕組み)
;;[ja]   3. 配列要素型の既定値化(開いたままなら Double)
;;[ja]   4. export — 残った型変数を Unknown-Type シンボルへ。生成器は
;;[ja]      それをテンプレート仮引数にする
;;[ja] 返り値: (環境 リスト, 最終式の型, unknown 一覧)
;;[ja] ============================================================
