...
設計行列は系列の関数なので、行を再標本化すると特徴量と目的変数の
対応が切れてしまいます。`bootstrap_windows` は代わりにブロック残差
ブートストラップを走らせます。一度当てはめ、残差を循環ブロックで
再標本化し (残差の系列依存を尊重するため)、`当てはめ値 + 再標本化
した残差` に対して当てはめ直します。Gram 行列は全再標本で共有される
ので、各再標本の費用はクロス積 1 パス O(n p) だけで、降下は 1 つの
バッチとして走ります — GPU では再標本ごとに 1 スレッドです。
...
窓 5 と 20 から作った目的変数では、どちらも頻度 1.0 で戻ります。
200 再標本で 1 秒足らずです。
