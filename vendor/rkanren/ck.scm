...
;; The substitution carries a hash of its own bindings beside the
;; association list, and walk reads the hash: profiled on a whole-program
;; type inference, nearly all of the time was assq walking a list of
;; thousands of bindings. The list stays because prefix-s reads deltas off
;; its tails, and the plain-alist representation stays legal because the
;; reifier builds one of its own -- every reader below accepts both.
(define (s-null? S) (null? (s-alist S)))

(define empty-s (cons '() (hasheq)))
